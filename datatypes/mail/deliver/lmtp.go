...
// transaction (section 4.2) - which is why Deliver returns one Event
...
// lmtpReply maps a Event to its LMTP reply line, with an enhanced
// status code (ENHANCEDSTATUSCODES, required by section 5).
func lmtpReply(ev Event) string {
	switch ev.Outcome {
	case mail.Accepted:
		return "250 2.1.5 OK"
	...
	default: // mail.Rejected: permanent (5yz), the client should bounce.
