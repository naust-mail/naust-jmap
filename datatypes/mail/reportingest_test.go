...
func newReportHarness(t *testing.T, mid string, opts ...DelivererOption) *reportHarness {
	t.Helper()
	...
	d := NewDeliverer(db, store, mapResolver{"john@example.com": testAccount},
		append([]DelivererOption{WithReportIngestion()}, opts...)...)
...
// the one event.
func (h *reportHarness) deliverReport(t *testing.T, raw string) DeliveryEvent {
	t.Helper()
...
func (h *reportHarness) getSubmission(t *testing.T) map[string]any {
	t.Helper()
	r := callSub(t, h.ts, inv("EmailSubmission/get", fmt.Sprintf(
...
func TestReportIngestDSNTerminal(t *testing.T) {
	...
	if ev.Outcome != Accepted || ev.EmailId == "" {
...
func TestReportIngestDestroyReleasesReports(t *testing.T) {
	...
	r := callSub(t, h.ts, inv("EmailSubmission/set", fmt.Sprintf(
