...
func submissionReportType() *descriptor.Type {
	return &descriptor.Type{
		...
		Properties: map[string]descriptor.Property{
			...
			"kind":         {Kind: descriptor.KindString, Internal: true}, // reportKindDSN or reportKindMDN
...
func loadReportRows(get func(string, jmap.Id) (objectdb.Object, error), index func(string, string, json.RawMessage) ([]jmap.Id, error), subId jmap.Id) ([]reportRow, error) {
	ids, err := index(TypeSubmissionReport, "submissionId", mustJSON(subId))
	if err != nil {
		return nil, err
	}
...
func (d *Deliverer) ingestReport(u *objectdb.Update, rep *inboundReport, blobID jmap.Id, now time.Time) (matched, deliver bool, err error) {
	subId, sub, exact, err := d.correlateSubmission(u, rep)
	if err != nil || subId == "" {
	...
	for _, c := range consumed {
		if _, err := u.Create(TypeSubmissionReport, objectdb.Object{
			"submissionId": mustJSON(subId),
			"kind":         mustJSON(c.kind),
			"recipient":    mustJSON(c.rcpt),
			"slot":         mustJSON(c.slot),
	...
	if dsChanged {
		newSub["deliveryStatus"] = mustJSON(ds)
	}
	...
	if err := u.Put(TypeEmailSubmission, subId, newSub); err != nil {
		return true, false, err
	}
...
func (d *Deliverer) correlateSubmission(u *objectdb.Update, rep *inboundReport) (jmap.Id, objectdb.Object, bool, error) {
	bySubmissionId := func(id jmap.Id) (objectdb.Object, error) {
		sub, err := u.Get(TypeEmailSubmission, id)
	...
	byMessageId := func(mid string) (jmap.Id, objectdb.Object, error) {
		ids, err := u.IdsWhereEqual(TypeEmailSubmission, "messageId", mustJSON(mid))
	...
	case reportKindDSN:
		...
		if d.msgIDFallback && rep.returnedMessageID != "" {
