...
// Searcher answers the text-search parts of Email/query the structural
// planner cannot: the RFC 8621 section 4.4.1 text FilterConditions. root's
// mail.Searcher (which also carries Snippet for SearchSnippet/get) is
// assignable to this narrower interface.
...
func EmailQueryHooks(db *objectdb.DB, searcher Searcher) *runtime.QueryHooks {
	return &runtime.QueryHooks{
		...
		GroupCompanion: record.TypeThread,
	}
}
...
func (EmailFilter) EnterRecord(ctx context.Context) context.Context {
	return parse.NewRecordContext(ctx)
}
...
func (f EmailFilter) ConditionSet(ctx context.Context, acct jmap.Id, name string, value json.RawMessage) ([]jmap.Id, bool, bool, error) {
	...
	case "before", "after":
		...
		var err error
		if name == "before" {
			ids, err = f.db.IdsWhereAtMost(ctx, acct, record.TypeEmail, "receivedAt", value, 0)
		} else {
			ids, err = f.db.IdsWhereAtLeast(ctx, acct, record.TypeEmail, "receivedAt", value, 0)
		}
		if err != nil {
	...
	ids, err := f.db.IdsWhereMember(ctx, acct, record.TypeEmail, prop, member)
	if err != nil {
		return nil, false, false, err
	}
...
func (f EmailFilter) MatchCondition(ctx context.Context, acct jmap.Id, obj objectdb.Object, name string, value json.RawMessage) (bool, error) {
	...
	case "inMailbox":
		...
		return emailstore.HasKey(obj["mailboxIds"], id), nil
	case "inMailboxOtherThan":
		var exclude []string
		...
		for mb := range emailstore.ObjectKeys(obj["mailboxIds"]) {
			if !ex[mb] {
				return true, nil
			}
		}
	...
	case "maxSize":
		return emailUint(obj, "size") < uintVal(value), nil
	case "hasKeyword":
		return emailstore.HasKey(obj["keywords"], keywordArg(value)), nil
	case "notKeyword":
		return !emailstore.HasKey(obj["keywords"], keywordArg(value)), nil
	case "hasAttachment":
...
func (f EmailFilter) threadKeyword(ctx context.Context, acct jmap.Id, obj objectdb.Object, keyword string) (all, some bool, err error) {
	tid := emailstore.ThreadIdOf(obj)
	ids, err := f.db.IdsWhereEqual(ctx, acct, record.TypeEmail, "threadId", record.MustJSON(tid), 0)
	if err != nil {
		return false, false, err
	}
	all = true
	for _, id := range ids {
		member, err := f.db.Get(ctx, acct, record.TypeEmail, id)
		if err != nil {
			return false, false, err
		}
		if emailstore.HasKey(member["keywords"], keyword) {
			some = true
		} else {
			all = false
		}
...
func (s emailSort) ParseSort(ctx context.Context, acct jmap.Id, raws []json.RawMessage) (func(a, b objectdb.Object) int, string, string) {
	...
	threadHas := func(obj objectdb.Object, keyword string, all bool) bool {
		...
		ids, err := s.db.IdsWhereEqual(ctx, acct, record.TypeEmail, "threadId", record.MustJSON(tid), 0)
		res := all
		if err == nil {
			for _, id := range ids {
				m, err := s.db.Get(ctx, acct, record.TypeEmail, id)
				if err != nil {
					continue
				}
				has := emailstore.HasKey(m["keywords"], keyword)
				if all && !has {
					res = false
				}
				if !all && has {
	...
	getSubject := func(obj objectdb.Object) string {
		...
		v := foldKey(emailstore.BaseSubject(record.StoredSubject(obj)))
		subjectCache[id] = v
		return v
	}
	getKeywords := func(obj objectdb.Object) map[string]bool {
		...
		v := emailstore.ObjectKeys(obj["keywords"])
		kwCache[id] = v
		return v
	}
...
func compareBool(a, b bool) int {
	return int(emailstore.B2i(a) - emailstore.B2i(b))
}
