...
func (h EmailCopy) Handle(ctx context.Context, call *runtime.Call) []jmap.Invocation {
	...
	fromState, err := h.Mat.DB.TypeState(ctx, a.FromAccountId, record.TypeEmail)
	if err != nil {
	...
	states, err := h.Mat.DB.Update(ctx, a.AccountId, func(u *objectdb.Update) error {
		state, err := h.Mat.DB.TypeState(ctx, a.AccountId, record.TypeEmail)
		if err != nil {
			return err
		}
		resp.OldState = state
	...
	resp.NewState = states[record.TypeEmail]
	if resp.NewState == "" {
	...
	if a.OnSuccessDestroyOriginal && len(copiedFrom) > 0 {
		...
		out = append(out, h.Proc.ImplicitSet(ctx, record.TypeEmail, struct {
			AccountId jmap.Id   `json:"accountId"`
			IfInState *string   `json:"ifInState"`
...
func (h EmailCopy) preflight(ctx context.Context, call *runtime.Call, a *emailCopyArgs, cid jmap.Id, raw json.RawMessage) (preparedCopy, error) {
	...
	original, err := h.Mat.DB.Get(ctx, a.FromAccountId, record.TypeEmail, fromId)
