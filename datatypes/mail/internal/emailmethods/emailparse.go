...
// EmailType is the Email descriptor (root's emailType()), needed only to
// check a requested property against the declared stored/fast set
// (parseAcceptsProperty) - passed in rather than looked up, since this
// package cannot import root back to call it.
type EmailParse struct {
	DB        *objectdb.DB
	Store     blob.Store
	Core      jmap.CoreCapabilities
	EmailType *descriptor.Type
}
...
func (h EmailParse) Handle(ctx context.Context, call *runtime.Call) []jmap.Invocation {
	...
	if raw, ok := all["properties"]; ok && !isNullRaw(raw) {
		...
		for _, name := range props {
			if !h.parseAcceptsProperty(name) {
	...
	for _, id := range blobIds {
		...
		msg, err := parse.ParseMessage(rc, parseCapture(props, compileBodyProps(args.bodyProperties), args))
		rc.Close()
		if err != nil {
			return runtime.Fail(call.CallID, jmap.ErrServerFail, err.Error())
		}
...
// header:From:asDate).
func (h EmailParse) parseAcceptsProperty(name string) bool {
	if name == "id" || computedFixed[name] {
		return true
	}
	if _, declared := h.EmailType.Properties[name]; declared {
		return true
	}
...
func parseCapture(props []string, plan bodyPropPlan, args getArgs) *parse.Capture {
...
func parseEmail(msg *parse.Parsed, blobID jmap.Id, size uint64, props []string, args getArgs) map[string]json.RawMessage {
	rec := emailstore.BuildEmailRecord(msg, emailstore.EmailMeta{BlobID: blobID, Size: size})
	...
	rec["id"] = record.JSONNull
	rec["mailboxIds"] = record.JSONNull
	rec["keywords"] = record.JSONNull
	rec["receivedAt"] = record.JSONNull
	rec["threadId"] = record.JSONNull
