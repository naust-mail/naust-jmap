...
var convenienceHeaders = map[string]struct {
	...
	"messageId":  {"Message-ID", emailstore.FormMessageIds},
	"inReplyTo":  {"In-Reply-To", emailstore.FormMessageIds},
	"references": {"References", emailstore.FormMessageIds},
	"sender":     {"Sender", emailstore.FormAddresses},
	"from":       {"From", emailstore.FormAddresses},
	"to":         {"To", emailstore.FormAddresses},
	"cc":         {"Cc", emailstore.FormAddresses},
	"bcc":        {"Bcc", emailstore.FormAddresses},
	"replyTo":    {"Reply-To", emailstore.FormAddresses},
	"subject":    {"Subject", emailstore.FormText},
	"sentAt":     {"Date", emailstore.FormDate},
}
...
func msgIDDomain(cfg GenConfig, obj map[string]json.RawMessage) string {
	...
	if raw, ok := obj["from"]; ok && json.Unmarshal(raw, &from) == nil && len(from) > 0 {
		if _, domain, ok := addr.Split(from[0].Email); ok && addr.TokenSafe(domain) {
			return domain
		}
	}
	return "localhost"
...
func serializeHeader(s headerSpec) ([]message.HeaderField, *jmap.SetError) {
	...
	for _, v := range vals {
		...
		if value == "" && s.form != emailstore.FormRaw && s.form != emailstore.FormText {
...
// a message header.
func serializeHeaderValue(field string, form emailstore.HeaderForm, raw json.RawMessage) (string, bool) {
	switch form {
	case emailstore.FormText:
		var v string
		if json.Unmarshal(raw, &v) != nil || hasCtl(v) {
			return "", false
		}
		return message.FoldValue(field, message.EncodeText(v)), true
	case emailstore.FormAddresses:
	...
	case emailstore.FormGroupedAddresses:
		...
		return message.FoldValue(field, atoms), true
	case emailstore.FormMessageIds:
		var ids []string
