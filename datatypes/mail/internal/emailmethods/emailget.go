...
func (c *EmailComputed) Resolve(ctx context.Context, acct jmap.Id, stored objectdb.Object, names []string, extra map[string]json.RawMessage) (map[string]json.RawMessage, error) {
	...
	for _, name := range names {
		switch name {
		case "headers":
			out[name] = record.MustJSON(headerList(msg.Msg.Headers))
		case "bodyStructure":
			out[name] = record.MustJSON(bodyPartJSON(msg.Msg.Root, plan))
		case "textBody":
			out[name] = record.MustJSON(bodyPartArray(msg.TextBody, plan))
		case "htmlBody":
			out[name] = record.MustJSON(bodyPartArray(msg.HtmlBody, plan))
		case "attachments":
			out[name] = record.MustJSON(bodyPartArray(msg.Attachments, plan))
		case "bodyValues":
			out[name] = record.MustJSON(bodyValues(msg, args))
		default:
...
func getCapture(names []string, plan bodyPropPlan, args getArgs) *parse.Capture {
	c := parse.NewCapture()
...
// have its blob.
func (c *EmailComputed) parseBlob(ctx context.Context, acct jmap.Id, stored objectdb.Object, want *parse.Capture) (*parse.Parsed, error) {
	var blobID jmap.Id
	...
	defer rc.Close()
	return parse.ParseMessage(rc, want)
}
...
func bodyPartArray(parts []*message.Part, plan bodyPropPlan) []json.RawMessage {
	...
	for _, p := range parts {
		out = append(out, record.MustJSON(bodyPartJSON(p, plan)))
	}
	return out
}
...
func bodyPartJSON(p *message.Part, plan bodyPropPlan) map[string]json.RawMessage {
	...
	for _, name := range plan.standard {
		switch name {
		case "partId":
			obj[name] = record.StrOrNull(record.NonEmpty(p.PartID))
		case "blobId":
			if multipart {
				obj[name] = record.JSONNull
			} else {
				...
				obj[name] = record.MustJSON(blob.IdFromDigest(p.Digest))
			}
		case "size":
			obj[name] = record.MustJSON(p.Size)
		case "name":
			obj[name] = ptrStrOrNull(p.Name)
		case "type":
			obj[name] = record.MustJSON(p.Type)
		case "charset":
		...
		case "language":
			if len(p.Language) == 0 {
				...
				obj[name] = record.MustJSON(p.Language)
			}
		case "location":
			obj[name] = ptrStrOrNull(p.Location)
		case "headers":
			obj[name] = record.MustJSON(headerList(p.Headers))
		case "subParts":
			if multipart {
				obj[name] = record.MustJSON(bodyPartArray(p.SubParts, plan))
			} else {
				obj[name] = record.JSONNull
			}
		}
...
func bodyValues(msg *parse.Parsed, args getArgs) map[string]bodyValue {
	...
	for id, p := range selected {
		value, problem, truncated, ok := msg.BodyValue(p)
		if !ok {
			continue // no value captured for this part: the capture did not ask
		}
		out[id] = bodyValue{
			Value:             value,
			IsEncodingProblem: problem || p.EncodingProblem,
...
// ptrStrOrNull duplicates record.StrOrNull; kept here as a private alias
// because callers in this file pass *string fields directly.
func ptrStrOrNull(s *string) json.RawMessage { return record.StrOrNull(s) }
