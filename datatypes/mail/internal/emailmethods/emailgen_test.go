...
// capture.
func generateAndParse(t *testing.T, obj map[string]json.RawMessage, cfg GenConfig, blobs map[jmap.Id][]byte) *parse.Parsed {
	t.Helper()
	...
	p, err := parse.ParseMessage(bytes.NewReader(buf.Bytes()), c)
	if err != nil {
		t.Fatal(err)
	}
	return p
...
func TestEmailGenerateEmptyBody(t *testing.T) {
	...
	if got, _, _, _ := p.BodyValue(p.TextBody[0]); got != "" {
...
func FuzzEmailGenerate(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, data []byte) {
		...
		if _, err := parse.ParseMessage(bytes.NewReader(buf.Bytes()), parse.NewCapture()); err != nil {
