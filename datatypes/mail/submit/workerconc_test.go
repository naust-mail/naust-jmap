...
func (s *crashSubmitter) Submit(_ context.Context, env Envelope, msg io.Reader) ([]Result, error) {
...
func (b *blockingSubmitter) Submit(ctx context.Context, env Envelope, msg io.Reader) ([]Result, error) {
	...
	out := make([]Result, len(env.Recipients))
	for i, r := range env.Recipients {
		out[i] = Result{Recipient: r.Email, Outcome: mail.Accepted, Reply: b.reply}
	}
	return out, nil
}
...
func TestWorkerDestroyDuringSendTolerated(t *testing.T) {
	...
	w.finalize(ctx, testAccount, jmap.Id(id), cA.stamp,
		[]Result{{Recipient: "jane@remote.example", Outcome: mail.Accepted, Reply: "250 gone"}})
	if _, err := db.Get(ctx, testAccount, mail.TypeEmailSubmission, jmap.Id(id)); err == nil {
...
func TestWorkerConcurrentFinalizeSupersededDropped(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		w.finalize(ctx, testAccount, jmap.Id(id), cA.stamp,
			[]Result{{Recipient: "jane@remote.example", Outcome: mail.Accepted, Reply: "250 current"}})
	}()
	go func() {
		defer wg.Done()
		w.finalize(ctx, testAccount, jmap.Id(id), staleToken,
			[]Result{{Recipient: "jane@remote.example", Outcome: mail.Rejected, Reply: "550 stale"}})
	}()
	wg.Wait()
...
func TestWorkerSuspendedClaimAbandonedByReread(t *testing.T) {
	...
	wA, err := NewWorker(newSubmissionQueue(db, store), aSub, cfg)
	if err != nil {
		t.Fatal(err)
	}
	wA.now = clock.now
	bSub := &fakeSubmitter{}
	wB, err := NewWorker(newSubmissionQueue(db, store), bSub, cfg)
	if err != nil {
		t.Fatal(err)
	}
	wB.now = clock.now
...
func TestWorkerReclaimDuringLiveTransmit(t *testing.T) {
	...
	bSub := &fakeSubmitter{respond: func(env Envelope) ([]Result, error) {
		out := make([]Result, len(env.Recipients))
		for i, r := range env.Recipients {
			out[i] = Result{Recipient: r.Email, Outcome: mail.Accepted, Reply: "250 2.0.0 B"}
		}
		return out, nil
	}}
	wA, err := NewWorker(newSubmissionQueue(db, store), aSub, cfg)
	...
	for ; rounds < maxRounds && finalCount() < N; rounds++ {
		...
		for k := 0; k < K; k++ {
			...
			go func() {
				...
				w, err := NewWorker(newSubmissionQueue(db, store), sub, cfg)
				if err != nil {
					return
				}
				w.now = clock.now
