...
// Worker tests: the queue engine driven deterministically
...
type fakeSubmitter struct {
	mu      sync.Mutex
	calls   []fakeCall
	respond func(env Envelope) ([]Result, error)
}
...
func (f *fakeSubmitter) Submit(_ context.Context, env Envelope, msg io.Reader) ([]Result, error) {
	...
	out := make([]Result, len(env.Recipients))
	for i, r := range env.Recipients {
		out[i] = Result{Recipient: r.Email, Outcome: mail.Accepted, Reply: "250 2.0.0 accepted"}
	}
	return out, nil
}
...
func newWorkerServer(t *testing.T, limits Limits, wcfg WorkerConfig) (*httptest.Server, *objectdb.DB, blob.Store, *Worker, *fakeSubmitter, *testClock) {
	t.Helper()
	...
	if err := srv.Capability(CapabilityURI).Advertise(struct{}{}, AccountCapabilityFor(limits)).Err(); err != nil {
		t.Fatal(err)
	}
...
func TestWorkerMixedRecipients(t *testing.T) {
	...
	fake.respond = func(env Envelope) ([]Result, error) {
		var out []Result
		for _, r := range env.Recipients {
			switch r.Email {
			case "ok@x.example":
				out = append(out, Result{r.Email, mail.Accepted, "250 2.1.5 ok"})
			case "gone@x.example":
				out = append(out, Result{r.Email, mail.Rejected, "550 5.1.1 no such user"})
			default:
				out = append(out, Result{r.Email, mail.TempFailed, "451 4.7.0 greylisted"})
			}
		}
		return out, nil
	}
...
func TestWorkerPartialResultsWithError(t *testing.T) {
	...
	fake.respond = func(env Envelope) ([]Result, error) {
		return []Result{{"ok@x.example", mail.Accepted, "250 2.1.5 ok"}},
...
func TestWorkerMissingResultTempFails(t *testing.T) {
	...
	fake.respond = func(env Envelope) ([]Result, error) {
		return []Result{{"ok@x.example", mail.Accepted, "250 2.1.5 ok"}}, nil
	}
	id := submitEnvelope(t, ts, identityId, emailId, `{
		"mailFrom":{"email":"john@example.com"},
...
func TestWorkerClaimParksDueIndex(t *testing.T) {
	...
	ids, err := db.IdsWhereAtMost(ctx, testAccount, record.TypeEmailSubmission, "nextAttemptAt",
		record.MustJSON(clock.now().UTC().Format(time.RFC3339)), 10)
	if err != nil {
		t.Fatal(err)
	}
	if len(ids) != 0 {
...
func TestWorkerClaimTokenCollisionResistant(t *testing.T) {
	...
	if _, err := db.Update(ctx, testAccount, func(u *objectdb.Update) error {
		obj, err := u.Get(record.TypeEmailSubmission, jmap.Id(id))
		if err != nil {
			return err
		}
		next := cloneObject(obj)
		next["nextAttemptAt"] = record.MustJSON(clock.now().UTC().Format(time.RFC3339))
		return u.Put(record.TypeEmailSubmission, jmap.Id(id), next)
	}); err != nil {
		t.Fatal(err)
	}
	...
	wA.finalize(ctx, testAccount, jmap.Id(id), cA.stamp,
		[]Result{{Recipient: "jane@remote.example", Outcome: mail.Accepted, Reply: "250 stolen"}})
	rec := subRecord(t, db, id)
	...
	wB.finalize(ctx, testAccount, jmap.Id(id), cB.stamp,
		[]Result{{Recipient: "jane@remote.example", Outcome: mail.Accepted, Reply: "250 2.0.0 ok"}})
...
func TestWorkerStaleFinalizeDropped(t *testing.T) {
	...
	_, err := db.Update(context.Background(), testAccount, func(u *objectdb.Update) error {
		...
		next := cloneObject(obj)
		next["claimedAt"] = record.MustJSON(current)
		return u.Put(record.TypeEmailSubmission, jmap.Id(id), next)
	})
	if err != nil {
		t.Fatal(err)
