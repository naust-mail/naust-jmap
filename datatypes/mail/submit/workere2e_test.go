...
func TestWorkerQueueTransitionInvariants(t *testing.T) {
	...
	fake.respond = func(env Envelope) ([]Result, error) {
		...
		for i, r := range env.Recipients {
			switch rng.Intn(3) {
			case 0:
				out[i] = Result{Recipient: r.Email, Outcome: mail.Accepted, Reply: "250 2.0.0 ok"}
			case 1:
				out[i] = Result{Recipient: r.Email, Outcome: mail.Rejected, Reply: "550 5.1.1 no such user"}
			default:
				out[i] = Result{Recipient: r.Email, Outcome: mail.TempFailed, Reply: "451 4.3.0 try later"}
			}
		}
		return out, nil
	}
