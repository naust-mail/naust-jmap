...
func (h submissionCreate) prepare(ctx context.Context, call *runtime.Call, acct, cid jmap.Id, raw json.RawMessage) (any, *jmap.SetError, error) {
	...
	email, err := h.db.Get(ctx, acct, mailrecord.TypeEmail, emailId)
	...
	identityView, err := mail.ReadIdentity(ctx, h.db, acct, identityId)
	if errors.Is(err, objectdb.ErrNotFound) {
	...
	identityEmail := identityView.Email
	...
	msg, err := parse.ParseMessage(rc, parse.NewCapture())
	rc.Close()
	if err != nil {
		return nil, &jmap.SetError{Type: "invalidEmail", Description: "message does not parse: " + err.Error()}, nil
	}
	fromAddrs, senderAddrs, badProps := parse.Check5322(msg.Msg.Headers)
	if len(badProps) > 0 {
	...
	for _, a := range fromAddrs {
		if !addr.IdentityAllows(identityEmail, a.Email) {
	...
	sendAtRaw := mailrecord.MustJSON(sendAt.UTC().Format(time.RFC3339))
	...
	record := objectdb.Object{
		"identityId":     mailrecord.MustJSON(identityId),
		"emailId":        mailrecord.MustJSON(emailId),
		"threadId":       email["threadId"],
		"envelope":       mailrecord.MustJSON(env),
		"sendAt":         sendAtRaw,
	...
	if mid := parse.MessageID(msg.Msg.Headers); mid != "" {
...
func (h submissionCreate) commit(u *objectdb.Update, prepared any) (jmap.Id, objectdb.Object, *jmap.SetError, error) {
	...
	if _, err := u.Get(mailrecord.TypeEmail, ps.emailId); errors.Is(err, objectdb.ErrNotFound) {
		return "", nil, invalidProp("emailId", "no such Email"), nil
	} else if err != nil {
		return "", nil, nil, err
	}
	id, err := u.Create(mailrecord.TypeEmailSubmission, ps.record)
	if err != nil {
		return "", nil, nil, err
	}
...
func (h submissionCreate) buildEnvelope(raw json.RawMessage, headers []message.HeaderField, fromAddrs, senderAddrs []message.Address, identityEmail string) (*subEnvelope, bool, *jmap.SetError) {
	...
	if !addr.IdentityAllows(identityEmail, mailFrom) {
		if addr.IsWildcard(identityEmail) {
	...
	for _, field := range []string{"To", "Cc", "Bcc"} {
		for _, instance := range parse.HeaderInstances(headers, field) {
...
func decodeEnvelope(raw json.RawMessage) (*subEnvelope, *jmap.SetError) {
	...
	if _, _, ok := addr.Split(env.MailFrom.Email); !ok {
...
func (h submissionCreate) checkRecipients(env *subEnvelope) *jmap.SetError {
	...
	var invalid []string
	for _, r := range env.RcptTo {
		if _, _, ok := addr.Split(r.Email); !ok {
