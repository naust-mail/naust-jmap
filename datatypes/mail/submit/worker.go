...
// queues work rings the Queue bell directly - lossless, so
...
// It consumes the Queue returned by Register;
...
// NewWorker builds a worker over the given queue and
...
func NewWorker(q *Queue, submitter Submitter, cfg WorkerConfig) (*Worker, error) {
	...
	if _, err := rand.Read(nb[:]); err != nil {
		return nil, fmt.Errorf("mail: Worker nonce: %w", err)
	}
	return &Worker{
		q:      q,
		submit: submitter,
...
// or Close).
func (w *Worker) pump(ctx context.Context, sub notify.Subscription) {
	for {
		...
		for _, ts := range changes {
			if _, ok := ts[record.TypeEmailSubmission]; ok {
				touched = true
				break
			}
		}
...
func (w *Worker) ProcessDue(ctx context.Context, limit int) (sent int, next time.Time, err error) {
	for {
		remaining := 0
		if limit > 0 {
...
func (w *Worker) healTags(ctx context.Context) error {
...
func (w *Worker) sweep(ctx context.Context, limit int) (sent int, nextDue time.Time, pending bool, err error) {
...
func (w *Worker) clearDrainedTag(ctx context.Context, acct jmap.Id) {
	w.q.db.Update(ctx, acct, func(u *objectdb.Update) error {
		ids, err := w.q.db.IdsWhereAtMost(u.Context(), acct, record.TypeEmailSubmission, "nextAttemptAt", nil, 1)
		if err != nil {
			return err
		}
		if len(ids) > 0 {
...
func (w *Worker) processAccount(ctx context.Context, acct jmap.Id, limit int) (sent int) {
	now := w.now()
	quota := w.cfg.BatchSize
	if limit > 0 && limit < quota {
		quota = limit
	}
	maxRaw := record.MustJSON(now.UTC().Format(time.RFC3339))
	ids, err := w.q.db.IdsWhereAtMost(ctx, acct, record.TypeEmailSubmission, "nextAttemptAt", maxRaw, quota)
...
func (w *Worker) claim(ctx context.Context, acct jmap.Id, id jmap.Id) (claimedRecord, bool) {
	now := w.now()
	...
	_, err := w.q.db.Update(ctx, acct, func(u *objectdb.Update) error {
		...
		obj, err := u.Get(record.TypeEmailSubmission, id)
		...
		obj = cloneObject(obj)
		obj["claimedAt"] = record.MustJSON(stamp)
		parked := now.Add(w.cfg.ClaimWindow).UTC().Format(time.RFC3339)
		obj["nextAttemptAt"] = record.MustJSON(parked)
		if err := u.Put(record.TypeEmailSubmission, id, obj); err != nil {
			return err
		}
		taken = true
		return nil
...
func (w *Worker) claimToken(now time.Time) string {
...
func (w *Worker) sendOne(ctx context.Context, acct jmap.Id, c claimedRecord) {
	var env subEnvelope
	var blobId jmap.Id
	...
	_, err := w.q.db.Update(ctx, acct, func(u *objectdb.Update) error {
		claimOK, lostClaim, lost = false, false, ""
		obj, err := u.Get(record.TypeEmailSubmission, c.id)
		if errors.Is(err, objectdb.ErrNotFound) {
	...
	for _, r := range env.RcptTo {
		if ds[r.Email].Delivered == "queued" {
			rcpts = append(rcpts, Recipient{Email: r.Email, Parameters: r.Parameters})
		}
	}
	var results []Result
	if len(rcpts) > 0 {
...
// error - an mail.Accepted result reports an irrevocable relay, and dropping
...
func (w *Worker) transmit(ctx context.Context, acct, id jmap.Id, env subEnvelope, blobId jmap.Id, rcpts []Recipient) []Result {
	...
	envid := string(id)
	params["ENVID"] = &envid
	results, err := w.submit.Submit(tctx, Envelope{
...
func mergeResults(rcpts []Recipient, results []Result, reply string) []Result {
	...
	for _, r := range rcpts {
		if !seen[r.Email] {
			results = append(results, Result{Recipient: r.Email, Outcome: mail.TempFailed, Reply: reply})
		}
	}
	return results
}
...
// per-recipient deliveryStatus per RFC 8621 section 7 (mail.Accepted ->
// delivered "unknown" - relaying to an external server MUST NOT be
// taken as final delivery; mail.Rejected -> "no"; mail.TempFailed stays "queued"
...
// (section 7.5).
func (w *Worker) finalize(ctx context.Context, acct jmap.Id, id jmap.Id, stamp string, results []Result) {
	...
	w.q.db.Update(ctx, acct, func(u *objectdb.Update) error {
		...
		obj = cloneObject(obj)
		obj["deliveryStatus"] = record.MustJSON(ds)
		obj["attempts"] = record.MustJSON(attempts)
		delete(obj, "claimedAt")
		if anyQueued {
			next := now.Add(w.backoff(attempts))
			obj["nextAttemptAt"] = record.MustJSON(next.UTC().Format(time.RFC3339))
			if anyAccepted {
				// Partially relayed: irrevocable, so no longer
				// cancelable, but the temp-failed recipients retry.
				obj["undoStatus"] = record.MustJSON(undoFinal)
			}
		...
		return u.Put(record.TypeEmailSubmission, id, obj)
	})
	if superseded {
...
func (w *Worker) backoff(attempts uint64) time.Duration {
	idx := int(attempts) - 1
