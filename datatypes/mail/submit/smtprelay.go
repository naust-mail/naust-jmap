...
func (s *SMTPRelay) Submit(ctx context.Context, env Envelope, msg io.Reader) ([]Result, error) {
	...
	if code/100 != 2 {
		...
		for i, r := range env.Recipients {
			out[i] = Result{Recipient: r.Email, Outcome: outcomeFor(code), Reply: reply}
		}
		return out, nil
	}
	...
	for i, r := range env.Recipients {
		...
		if err != nil {
			results[i] = Result{Recipient: r.Email, Outcome: mail.Rejected, Reply: "501 5.5.4 " + err.Error()}
			continue
		}
		...
		results[i] = Result{Recipient: r.Email, Outcome: outcomeFor(code), Reply: reply}
		if code/100 == 2 {
			accepted++
		}
	}
	...
	for i := range results {
		if results[i].Outcome != mail.Accepted {
			continue
		}
		if code/100 != 2 {
...
func outcomeFor(code int) mail.Outcome {
	switch code / 100 {
	case 2:
		return mail.Accepted
	case 4:
		return mail.TempFailed
	default:
		return mail.Rejected
	}
}

func rejectAll(rcpts []Recipient, reply string) []Result {
	out := make([]Result, len(rcpts))
	for i, r := range rcpts {
		out[i] = Result{Recipient: r.Email, Outcome: mail.Rejected, Reply: reply}
	}
	return out
}
...
func buildMailCmd(env Envelope, ext map[string]string, dsnOK bool) (string, error) {
...
func buildRcptCmd(r Recipient, dsnOK bool) (string, error) {
	if !addr.WireSafe(r.Email) {
