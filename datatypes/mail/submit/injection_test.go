...
func TestRelayAddressInjectionRejected(t *testing.T) {
	for _, bad := range injectionAddrs {
		if _, err := buildMailCmd(Envelope{MailFrom: bad}, nil, false); err == nil {
			t.Errorf("buildMailCmd accepted %q", bad)
		}
		cmd, err := buildRcptCmd(Recipient{Email: bad}, false)
		if err == nil {
	...
	if _, err := buildMailCmd(Envelope{MailFrom: "john@example.com"}, nil, false); err != nil {
...
func TestRelayParamInjectionRejected(t *testing.T) {
	for _, bad := range injectionParams {
		if _, err := buildMailCmd(
			Envelope{MailFrom: "john@example.com", MailParameters: map[string]*string{"RET": ptr(bad)}},
			dsnAdvertised, true); err == nil {
			t.Errorf("buildMailCmd accepted RET=%q", bad)
		}
		if _, err := buildRcptCmd(
			Recipient{Email: "jane@remote.example", Parameters: map[string]*string{"NOTIFY": ptr(bad)}},
			true); err == nil {
		...
		cmd, err := buildRcptCmd(
			Recipient{Email: "jane@remote.example", Parameters: map[string]*string{"ORCPT": ptr(orcpt)}},
			true)
		if err == nil {
	...
	if _, err := buildMailCmd(
		Envelope{MailFrom: "john@example.com", MailParameters: map[string]*string{"RET": ptr("HDRS")}},
		dsnAdvertised, true); err != nil {
		t.Errorf("buildMailCmd rejected a valid RET: %v", err)
	}
	if _, err := buildRcptCmd(
		Recipient{Email: "jane@remote.example", Parameters: map[string]*string{"NOTIFY": ptr("SUCCESS,FAILURE")}},
		true); err != nil {
		t.Errorf("buildRcptCmd rejected a valid NOTIFY: %v", err)
	}
	if _, err := buildRcptCmd(
		Recipient{Email: "jane@remote.example", Parameters: map[string]*string{"ORCPT": ptr("rfc822;jane@remote.example")}},
		true); err != nil {
...
func TestAppendParamKeyInjectionRejected(t *testing.T) {
	for _, bad := range injectionParams {
		if _, err := buildMailCmd(
			Envelope{MailFrom: "john@example.com", MailParameters: map[string]*string{bad: ptr("v")}},
			nil, false); err == nil {
			t.Errorf("buildMailCmd accepted parameter key %q", bad)
		}
		if _, err := buildMailCmd(
			Envelope{MailFrom: "john@example.com", MailParameters: map[string]*string{bad: nil}},
			nil, false); err == nil {
	...
	if _, err := buildMailCmd(
		Envelope{MailFrom: "john@example.com", MailParameters: map[string]*string{"X-CUSTOM": ptr("v")}},
		nil, false); err != nil {
