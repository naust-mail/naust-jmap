...
// Recipient is one envelope recipient of an outbound message:
...
// Envelope is the SMTP envelope for one transmission attempt:
...
// Result is the fate of one recipient in one transmission
// attempt. Outcome reuses the delivery classification (mail.Accepted -> 2xx,
// mail.Rejected -> permanent 5xx, mail.TempFailed -> transient 4xx); Reply is the
...
// as on a normal return - an mail.Accepted result reports an irrevocable
...
type Submitter interface {
	Submit(ctx context.Context, env Envelope, msg io.Reader) ([]Result, error)
}
