...
func newSubmissionServer(t *testing.T, limits Limits) (*httptest.Server, *objectdb.DB, blob.Store, *submissionPolicy) {
	t.Helper()
...
func submissionServer(t *testing.T) (*httptest.Server, *objectdb.DB, blob.Store, *submissionPolicy) {
	return newSubmissionServer(t, DefaultLimits())
}
...
func callSub(t *testing.T, ts *httptest.Server, calls ...jmap.Invocation) *jmap.Response {
	t.Helper()
	req := map[string]any{
		"using":       []string{jmap.CoreCapability, mail.CapabilityURI, CapabilityURI},
		"methodCalls": calls,
	}
...
func TestEmailSubmissionSendFlow(t *testing.T) {
	...
	stored, err := db.Get(context.Background(), testAccount, record.TypeEmailSubmission, jmap.Id(subId))
	if err != nil {
		t.Fatal(err)
	}
...
func TestEmailSubmissionFutureRelease(t *testing.T) {
	...
	for _, params := range []string{
		fmt.Sprintf(`{"HOLDFOR":"%d"}`, DefaultLimits().MaxDelayedSend+1),
	...
	ts2, db2, store2, _ := newSubmissionServer(t, Limits{MaxRecipients: 10, MaxMessageBytes: 1 << 20})
...
func TestEmailSubmissionCancel(t *testing.T) {
	...
	_, err = db.Update(context.Background(), testAccount, func(u *objectdb.Update) error {
		obj, err := u.Get(record.TypeEmailSubmission, jmap.Id(finalId))
		if err != nil {
			return err
		}
		next := cloneObject(obj)
		next["undoStatus"] = json.RawMessage(`"final"`)
		if err := u.Put(record.TypeEmailSubmission, jmap.Id(finalId), next); err != nil {
			return err
		}
		obj, err = u.Get(record.TypeEmailSubmission, jmap.Id(claimedId))
		if err != nil {
			return err
		}
		next = cloneObject(obj)
		next["claimedAt"] = record.MustJSON(time.Now().UTC().Format(time.RFC3339))
		return u.Put(record.TypeEmailSubmission, jmap.Id(claimedId), next)
	})
	if err != nil {
		t.Fatal(err)
	}
...
func TestEmailSubmissionQuery(t *testing.T) {
	...
	sub, err := db.Get(context.Background(), testAccount, record.TypeEmailSubmission, jmap.Id(subA))
	if err != nil {
		t.Fatal(err)
	}
