...
// Postgres backend to prove blobs larger than a single comfortable value can
// be stored, read back, survive a reopen, and have their crash-orphaned
// pieces reclaimed - none of which needs any Postgres-specific blob code.
// Ported from drivers/sqlite/chunkstore_integration_test.go; this is the
// actual point of the Postgres driver - a second real Backend running the
// same code paths to catch anything that only happened to hold for
// sqlite/memory.
...
func rows(t *testing.T, s *Store) int {
	t.Helper()
	var n int
	if err := s.pool.QueryRow(context.Background(), `SELECT count(*) FROM kv`).Scan(&n); err != nil {
		t.Fatalf("count: %v", err)
	}
	return n
}

// TestChunkstoreOverPostgres streams a multi-piece blob (larger than one
// piece) through Postgres and reads it back byte-for-byte at its content
...
func TestChunkstoreOverPostgres(t *testing.T) {
	be := openTestDB(t)
	dsn := be.pool.Config().ConnString()
	...
	// Reopen the database: the blob must still be there.
	if err := be.Close(); err != nil {
		t.Fatal(err)
	}
	be2, err := Open(ctx, dsn)
	if err != nil {
		t.Fatal(err)
	}
	defer be2.Close()
...
// TestChunkstoreSweepOverPostgres simulates a crash mid-upload: pieces and a
...
func TestChunkstoreSweepOverPostgres(t *testing.T) {
	be := openTestDB(t)
	dsn := be.pool.Config().ConnString()
